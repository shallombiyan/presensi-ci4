...
<div class="d-flex justify-content-center">
  ...
  <div class="card">
    ...
    <?php else: ?>
      <div class="card-body text-center">
        <?php
          if ($cek_presensi_keluar): // Jika sudah presensi keluar hari ini
        ?>
          <h5 class="text-center"><i class="fas fa-check-circle"></i> Anda sudah melakukan presensi keluar</h5>
        <?php else: ?>
          <div class="parent-clock">
          ...
          <form method="post" action="<?= base_url('pegawai/presensi_keluar/'. $ambil_presensi_masuk['id']) ?>">
            <?php
              // Mengatur zona waktu berdasarkan lokasi presensi
              if ($lokasi_presensi['zona_waktu'] == 'WIB') {
                date_default_timezone_set('Asia/Jakarta');
              } elseif ($lokasi_presensi['zona_waktu'] == 'WITA') {
                date_default_timezone_set('Asia/Makassar');
